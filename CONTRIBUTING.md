...
agent-101 is an educational repo. Its goal is to teach how an agent harness works, in 18 numbered files of Python you can read on a flight. Contributions that *advance that goal* are welcome. Contributions that turn it into a framework are not.
...
- **Ports to other languages.** Open a PR adding your link to the README's "Notable ports" table. Naming convention: `agent-101-go`, `agent-101-rust`, `agent-101-ts`.
...
- Frameworks. No `pip install agent-101`. No factory classes. No abstract base classes for tools.
