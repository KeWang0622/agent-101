...
2. `tail -f ~/.agent-zero-to-hero/sessions/<id>.jsonl` while running. Watch each turn append.
   *(Note: production `agent.py` namespaces by cwd — `~/.agent-zero-to-hero/projects/<slug>/<id>.jsonl`.)*
